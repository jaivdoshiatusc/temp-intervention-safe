...
class BlockerModel(nn.Module):
    def __init__(self, action_size):
        ...
        # Input: (1, 44, 120)
        self.conv1 = nn.Conv2d(1, 4, kernel_size=5, stride=2)  # Output: (4, 20, 58)
        self.conv2 = nn.Conv2d(4, 8, kernel_size=3, stride=2)  # Output: (8, 9, 28)

        # Initialize and calculate the conv output size dynamically
        self._initialize_weights()
        test_input = torch.zeros(1, 1, 44, 120)  # Dummy input: (batch=1, channels=1, H=44, W=120)
    ...
    def conv_layers(self, x):
        x = torch.relu(self.conv1(x))  # First convolution layer
        x = torch.relu(self.conv2(x))  # Second convolution layer
        return x
    ...
    def _initialize_weights(self):
        # Optionally initialize weights here if needed
        pass
...
class SpaceInvadersBlockerTrainer:
