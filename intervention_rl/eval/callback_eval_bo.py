...
class BreakoutEvalCallback(BaseCallback):
    PLAY_AREA = [8, 152]  # Play area in Pong

    def __init__(self, cfg, eval_env, eval_freq, eval_seed, gif_freq, n_eval_episodes, new_action, verbose=1):
        super(BreakoutEvalCallback, self).__init__(verbose)
    ...
    def create_frame(self):
        ...
        blocker_x_position = self.PLAY_AREA[1] - self.blocker_clearance  # Example: blocker at x = clearance
        catastrophe_x_position = self.PLAY_AREA[1] - self.catastrophe_clearance  # Example: catastrophe at x = clearance

        # Vertical line for blocker zone
        cv2.line(frame, (blocker_x_position, 0), (blocker_x_position, frame.shape[0]), line_color, 1)

        # Vertical line for catastrophe zone
        cv2.line(frame, (catastrophe_x_position, 0), (catastrophe_x_position, frame.shape[0]), line_color, 1)
